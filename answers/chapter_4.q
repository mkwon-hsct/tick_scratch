...
// load trades.q first
exercise_2: ![trades; ((within; `date; 2013.07.01 2013.07.03); (in; `sym; enlist `AAPL`MSFT)); 0b; `sym`size!(({`$3#/:string x}; `sym); (xbar; 10000; `size))];
...
// expense_orig: expense;

// You must create `hdb/` first by `mkdir hdb`.
// Then start a q process in `hdb/` directry.

// expense: delete date from expense_orig
exercise_3: .Q.dpft[`:.; 2019.10.05; `person; `expense];
